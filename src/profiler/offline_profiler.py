...
def parse_args():
	...
	# Profiler specific
	parser.add_argument('--profiler-src', default="/synergy-master/src/", type=str,
	...
	parser.add_argument('--local-disk', default="/home/gaoxiaoqing", type=str,
...
def launch_job(job_name, container, cpu, memory, script, script_args, out_dir):
	...
	cmd = "nvidia-docker run " + \
				" --ipc={}".format("host") + \
				" --mount src={},target={},type={}".format("/home/gaoxiaoqing", args.container_mnt, \
				"bind")+\
				" -it" + \
				" --rm" + \
				" --network={}".format("host") + \
...
def profile_cpu(num_cpus, mem):
	...
	cpus_to_profile = [8, 4, 2, 1]
	mems_to_profile = [100, 60, 40, 20]
	point = 0
	# while True:
	for cpu_to_profile in cpus_to_profile:
		for mem_to_profile in mems_to_profile:
		#cpu_to_profile = cpus_to_profile[point]
			start = time.time()
			job_name = args.job_name + "_cpu_" + str(cpu_to_profile) + "_mem_" + str(mem_to_profile)
			cur_res = launch_job(job_name, args.docker_img, cpu_to_profile, mem_to_profile, args.training_script, args.training_script_args, "./") 
		#cur_res = launch_job_dummy_oi(cpu_to_profile)
			iter_times[cpu_to_profile] = cur_res
			cur_res = cur_res * num_iters
			if point == 0:
				res = cur_res
			print("Epoch time = {}, num_iters = {}, job_name = {}".format(cur_res, num_iters, job_name))
...
def main():
	...
	#declare memory max available
	mem = 100
	
	cpu_profile, iter_times = profile_cpu(args.cpu, mem)
