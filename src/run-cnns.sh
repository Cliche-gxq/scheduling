...
for arch in 'vgg11'; do
	for workers in $WORKER; do
		for batch in 32; do
      job_name="${arch}_${batch}_1_cpu_imagenet"
			echo "Now running $arch for $workers workers and $batch batch"
      start=$SECONDS
      python profiler/offline_profiler.py --job-name $job_name --cpu 8 --num-gpus 1 -b $batch --docker-img=jayashreemohan/synergy_dali:latest ../models/image_classification/pytorch-imagenet.py -a $arch --dali --amp --dali_cpu --max-iterations 50 --synergy --noeval --workers $workers -b $batch --data '/datadrive/ILSVRC2012'
...
for arch in 'alexnet' 'shufflenet_v2_x0_5' 'mobilenet_v2' 'squeezenet1_0' 'resnet50' 'vgg11' 'resnet18'; do
	for workers in $WORKER; do
      ...
      job_name="${arch}_gpu_openimages"
			echo "Now running $arch for $workers workers and $batch batch"
      start=$SECONDS
      python profiler/offline_profiler.py --job-name $job_name --cpu 24 --num-gpus 1 -b $batch --docker-img=jayashreemohan/synergy_dali:latest ../models/image_classification/pytorch-imagenet.py -a $arch --dali --amp --max-iterations 50 --workers 3 -b $batch --data '/datadrive/ILSVRC2012/'
      duration=$(( SECONDS - start ))
			echo "RAN $arch for $workers workers, $batch for $duration s"
			sleep 2
      ...
      job_name="${arch}_cpu_openimages"
			echo "Now running $arch for $workers workers and $batch batch"
      start=$SECONDS
      python profiler/offline_profiler.py --job-name $job_name --cpu 24 --num-gpus 1 -b $batch --docker-img=jayashreemohan/synergy_dali:latest ../models/image_classification/pytorch-imagenet.py -a $arch --dali --amp --dali_cpu --max-iterations 50 --workers 3 -b $batch --data '/datadrive/ILSVRC2012'
      duration=$(( SECONDS - start ))
			echo "RAN $arch for $workers workers, $batch for $duration s"
			sleep 2
